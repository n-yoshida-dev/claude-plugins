...
HANDOFF / PLAN / SPEC / TODO / KNOWLEDGE の4ファイル体系と、`frontend/`（React + TS）/ `backend/`（Go）の
ディレクトリ構成を前提にしている。
...
| フック（SessionStart） | `hooks/session-briefing.sh` | `TODO.md` の `- [ ]` 行を先頭12件まで context に流し込む |
| スキル | `/apps-workflow:handoff` | セッションの区切りに HANDOFF / TODO / KNOWLEDGE を更新する |
